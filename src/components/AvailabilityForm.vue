  ...
  <div class="container mt-5">
    ...
    <form @submit.prevent="createAvailability">
      <div class="mb-3">
        ...
        <input v-model="startTime" type="datetime-local" class="form-control" id="startTime" required>
      </div>
      <div class="mb-3">
        <label for="endTime" class="form-label">Fim</label>
        <input v-model="endTime" type="datetime-local" class="form-control" id="endTime" required>
      </div>
  ...
  methods: {
    async createAvailability() {
      try {
        ...
        this.error = error.response?.data?.message || 'Erro ao cadastrar horário';
      }
    }
  }
};
